...
Legalcode MCP `legalcode_search` call with the appropriate `sourceTypes` covers the source,
...
source is reachable through Legalcode MCP `legalcode_search`; "Direct" means the
skill must fall back to the API/SPARQL
shape described above.

| Source                         | law sourceType | case sourceType | guidance sourceType | Direct fallback          |
| ------------------------------ | -------------- | --------------- | ------------------- | ------------------------ |
