---
name: legalcode-triage-nda
description: Rapidly triage incoming NDAs — classify as GREEN (standard approval), YELLOW (counsel review),
  or RED (significant issues) with confidence-scored, auditable analysis. Use when a new NDA arrives from
  sales, business development, procurement, or a counterparty, when assessing NDA risk level before signature,
  when deciding whether an NDA needs full counsel review, or when routing NDAs through an in-house legal
  workflow. Covers AI/ML data training provisions, DTSA whistleblower immunity compliance, residuals clause
  analysis, trade secret duration assessment, and stakeholder impact mapping. Jurisdiction-agnostic with
  localization markers for any governing law. Supports playbook-based or general-standard screening (including
  oneNDA standard-form benchmarking). Produces a structured triage report with Glass Box audit trail.
...
- AI/ML data training and generative AI provisions screening
- DTSA whistleblower immunity notice compliance (US)
- Trade secret duration and survival period analysis
- Playbook-based or general-standard screening (including oneNDA benchmarking)
...
- AI-specific data protection rules that may affect confidentiality provisions
...
2. **Business context**: What is the purpose of this NDA?
   - Options: Exploratory / pre-sales discussions, Vendor evaluation, M&A due diligence,
     Technology evaluation / proof of concept, Employment / contractor, Joint venture /
     partnership, Other
...
5. **AI/technology concerns**: Does your organization use AI/ML tools that may process
   shared information?
   - Options: Yes — we need AI-specific screening, No — standard screening is fine,
     Not sure — flag AI issues if present
   - _Why this matters_: AI/ML data training restrictions are an emerging area. If your
     organization uses AI tools, restrictions on AI processing of confidential information
...
- AI/data training restrictions applicable to the organization
...
- No AI/ML training rights over confidential information
...
- DTSA whistleblower immunity notice required (US-governed NDAs)

These defaults are aligned with the [oneNDA standard-form](https://www.onenda.org/)
provisions and general market practice. oneNDA is adopted by 6,000+ organizations for
~10M agreements annually and represents a well-accepted market baseline.
...
- Search for jurisdiction-relevant statutes governing: trade secret protection,
  ...
  relief standards, non-compete/non-solicitation enforceability in NDAs, AI data
  processing restrictions
- Save results to a local temp file (`/tmp/legalcode-triage-nda-authority.md`)
...
Evaluate the NDA against each of the 11 screening criteria systematically:
...
- [ ] **Purpose clause**: Does the NDA state a clear purpose for the disclosure? Is it
...
- [ ] **Marking requirements**: If marking is required, is it workable? (Written marking
      within 30 days of oral disclosure is standard). Note: failure to mark under a marking
      requirement can forfeit NDA protection — courts enforce marking obligations strictly
      [VERIFY]
...
- [ ] **Residual information carveout**: If present, check scope (see Criterion 9)
...
Poland; DTSA in the US; EU Trade Secrets Directive 2016/943). Where local law provides a
statutory definition of trade secrets, note whether the NDA's definition is broader,
narrower, or aligned.
...
- [ ] **No AI/ML training restriction conflicts**: If AI restrictions exist, check they
      are workable (see Criterion 11)
...
**DTSA Whistleblower Immunity (US-governed NDAs)**: Under 18 U.S.C. § 1833(b), NDAs
governed by US law should include a notice that an individual will not be held criminally
or civilly liable under any federal or state trade secret law for disclosing a trade
secret in confidence to a government official or attorney for the purpose of reporting a
suspected violation of law, or in a complaint or other document filed under seal in a
lawsuit. Failure to include this notice means the employer cannot recover exemplary
damages or attorneys' fees in a trade secret misappropriation action. [VERIFY]

[JURISDICTION-SPECIFIC] The compelled disclosure carveout procedure varies by jurisdiction.
Some jurisdictions require prior notice to the disclosing party; others do not permit
prior notice in certain regulatory contexts (e.g., whistleblower protections, regulatory
investigations). Ensure the NDA's compelled disclosure mechanism is compatible with the
governing law.
...
- [ ] **AI tools**: If AI restrictions apply, check whether use of AI-powered tools for
      internal analysis of confidential information is permitted (see Criterion 11)
...
- [ ] **Not perpetual**: Avoid indefinite or perpetual confidentiality obligations
      ...
      trade secrets)
- [ ] **Trade secret bifurcation**: If the NDA bifurcates trade secrets from other
      confidential information for duration purposes, is the mechanism well-drafted?
...
protection. Under the DTSA (US), trade secrets are protected for as long as they maintain
secrecy; the NDA should not inadvertently shorten this statutory protection. [VERIFY]
...
- [ ] **Electronic copies**: Addresses electronically stored information, including backups
      and archived copies that may not be practicably deletable
- [ ] **AI model residuals**: If AI tools have processed confidential information, does the
      return/destruction clause address learned model parameters or training artifacts?
...
- [ ] **Not one-sided**: Remedies provisions apply equally to both parties (in mutual
      NDAs)
- [ ] **No consent to jurisdiction for injunction**: Avoid clauses that consent to specific
      courts having jurisdiction solely for injunctive relief — this can create strategic
...
England per Cavendish Square v Makdessi [2015] UKSC 67; miarkowanie per art. 484 s.2 KC
in Poland; unconscionability in the US). [VERIFY] specific standards for the governing
law.
...
- [ ] **No residuals clause** (or narrowly scoped): If present, must be limited to
      ...
      Note: courts have found that overly broad residual language can constitute an actual
      trade secret license [VERIFY]
...
- [ ] **No data collection/usage rights**: NDA should not grant data analytics, profiling,
      or aggregation rights over the disclosed information
...
the US, enforceability varies by state — California (Cal. Bus. & Prof. Code § 16600) and
several other states ban most non-competes; the FTC's 2024 non-compete rule was vacated
but signals regulatory direction. [VERIFY] for the governing law.
...
- [ ] **Cross-border enforcement**: For international NDAs, consider whether the chosen
      jurisdiction's judgments are enforceable where the counterparty has assets

#### Criterion 11: AI/Technology Provisions

- [ ] **AI training restrictions**: Does the NDA address use of confidential information
      for AI/ML model training? If present, are restrictions reasonable and workable?
- [ ] **Generative AI output**: Does the NDA address whether AI-generated outputs based
      on confidential information are covered by the confidentiality obligations?
- [ ] **AI tool usage**: Does the NDA restrict or permit use of AI-powered tools
      (summarizers, analyzers, chatbots) for processing confidential information?
- [ ] **Data scraping/crawling**: Are there restrictions on automated collection or
      processing of disclosed information?
- [ ] **Reverse engineering**: Does the NDA address reverse engineering of shared
      technology, algorithms, or data models?
- [ ] **Model decommissioning**: If AI models are trained on confidential information,
      does the return/destruction clause address model weights and learned parameters?

**Screening logic for AI provisions:**

- If AI provisions are **absent** and user indicated AI concerns: FLAG (YELLOW) — recommend
  adding AI-specific language before signing
- If AI provisions are **present and reasonable**: PASS — note presence and scope
- If AI provisions **unreasonably restrict** internal AI tool usage for analyzing the NDA
  itself or for standard business operations: FLAG (YELLOW) — may need negotiation
- If AI provisions **grant the counterparty rights** to train AI models on your
  confidential information: FAIL (RED) — this is effectively an IP license
...
- No AI/technology provisions that create unworkable restrictions or IP licenses
- DTSA whistleblower notice present (if US-governed) or not required (non-US)
...
- AI provisions absent when user has AI concerns — recommend adding before signing
- DTSA whistleblower notice absent in US-governed NDA (can be added without difficulty)
- AI restrictions present but may be over-inclusive for standard business operations
...
- **AI training rights** granted to the counterparty over your confidential information
- **Data collection/aggregation rights** that persist beyond the NDA term
...
- **Risk if too broad**: Effectively grants a license to use the disclosing party's
  confidential information for any purpose. Courts have found broad residual language
  constitutes an actual trade secret license. [VERIFY]
...
**Issue: Missing DTSA Whistleblower Immunity Notice (US NDAs)**

- **Standard position**: All NDAs governed by US law should include the DTSA whistleblower
  immunity notice per 18 U.S.C. § 1833(b). Without it, the employer forfeits exemplary
  damages and attorneys' fees in trade secret misappropriation actions. [VERIFY]
- **Redline approach**: Add standard DTSA notice language.
- **Priority**: Tier 2 (YELLOW) — easy to add, no resistance expected.

**Issue: AI/ML Training Restrictions**

- **Standard position**: Confidential information should not be used for AI/ML model
  training without explicit consent. If the counterparty's NDA grants AI training rights,
  this is effectively an IP license.
- **Risk if absent**: AI tools may inadvertently process confidential information through
  cloud-based services, creating data leakage risks.
- **Redline approach**: Add explicit prohibition on use of confidential information for
  AI/ML training, with reasonable carveout for internal use of AI tools for analyzing
  and processing the information for the stated purpose.
- **Priority**: Tier 2 (YELLOW) if absent and AI concerns exist; Tier 1 (RED) if
  counterparty seeks AI training rights over your information.

### Step 8: Stakeholder Impact Mapping

For NDAs involving complex business relationships, identify all affected stakeholders:

| Stakeholder       | Role             | Affected Provisions                 | Impact     | Action Required                    |
| ----------------- | ---------------- | ----------------------------------- | ---------- | ---------------------------------- |
| [Party A]         | [Disclosing]     | All                                 | Primary    | Sign / Negotiate                   |
| [Party B]         | [Receiving]      | All                                 | Primary    | Sign / Negotiate                   |
| [Employees]       | Internal         | Permitted disclosures, non-solicit  | Indirect   | Ensure NDA doesn't restrict hiring |
| [Contractors]     | Third party      | Permitted disclosures               | Indirect   | Flow-down obligations              |
| [AI/tech vendors] | Third party      | AI provisions, data processing      | Indirect   | May need separate DPA              |
| [Affiliates]      | Related entities | Permitted disclosures, scope        | Indirect   | Confirm coverage extends           |
| [Regulator]       | Oversight        | Compelled disclosure, whistleblower | Compliance | Ensure carveouts permit compliance |

This mapping prevents the common failure of reviewing an NDA as a two-party document when
it affects multiple stakeholders with different interests.

### Step 9: Negotiation Priority Framework
...
- AI training rights granted to counterparty over your confidential information
...
- DTSA whistleblower notice addition (US)
- AI-specific confidentiality provisions
...
- Non-exclusive to exclusive jurisdiction upgrade
...
| ---------- | ------------- | --------------------------------------------------------------------------------------- | ------------------------------------- |
| **Tier 1** | Must-Haves    | Missing carveouts, non-compete, IP assignment, broad residuals, AI training rights      | Non-negotiable; escalate if rejected  |
| **Tier 2** | Should-Haves  | Overbroad definition, term adjustments, retention exception, DTSA notice, AI provisions | Push firmly; accept fallback          |
| **Tier 3** | Nice-to-Haves | Governing law preference, minor asymmetries, dispute mechanism                          | Concede strategically for Tier 2 wins |
...
**DO**: Execute the 11-criteria screening. Classify GREEN/YELLOW/RED. Generate findings
with legal basis and suggested fixes. Assign negotiation priorities. Map stakeholder
impacts.

**CHECK**: Run the Citation Quality Gates. Verify legal claims in the triage rationale.
For RED classifications, run the Self-Interrogation. Confirm all 11 criteria addressed.

**ACT**: If the NDA reveals a new pattern (e.g., a clause structure becoming common in
the market, new AI provisions emerging), note it for playbook update. If the
organization's standard form NDA is missing a protection that this NDA exposed, flag
for standard form revision.
...
glass_box:
  ...
  criteria_summary:
    ...
    ai_technology: "PASS / FLAG / NOT SCREENED — [brief note]"
  stakeholder_mapping: "Completed / Not applicable"
  dtsa_whistleblower_notice: "Present / Absent / Not applicable (non-US)"
...
15. **Ignoring AI/technology provisions entirely** — As of 2025, AI-specific NDA
    provisions are an emerging market expectation. An NDA that allows counterparties to
    feed your confidential information into AI training datasets creates a novel IP
    leakage vector. Screen for it.

16. **Treating marking requirements as boilerplate** — Courts enforce marking obligations
    strictly. If an NDA requires marking and a disclosing party fails to mark information,
    the information may fall outside the NDA's protection entirely. This is a real
    enforcement risk, not a formality.

17. **Confusing "mutual" with "balanced"** — A mutual NDA imposes obligations on both
    parties. That does not mean the obligations are balanced. One party may have broader
    definitions, longer terms, or fewer carveouts than the other. Screen for asymmetry
    even in mutual NDAs.
...
- In Step 4, search for jurisdiction-relevant statutes governing trade secrets,
  confidentiality, penalties, injunctions, restrictive covenants, and AI data processing
- Save the most relevant results to `/tmp/legalcode-triage-nda-authority.md`
- Reference verified authority throughout the screening
...
| 11  | AI/Technology Provisions    | [PASS/FLAG/FAIL/NOT SCREENED] | [details] |
...
| Stakeholder   | Affected Provisions | Impact   | Action   |
| ------------- | ------------------- | -------- | -------- |
| [stakeholder] | [provisions]        | [impact] | [action] |
...
3. Add jurisdiction-specific screening criteria (e.g., English law penalty doctrine,
   Polish kary umowne analysis, US state-specific non-compete rules, DTSA compliance)
...
- **England & Wales**: `legalcode-nda-triage-ew` — American Cyanamid test, Cavendish
  Square penalty doctrine, Trade Secrets Regulations 2018, restraint of trade analysis
- **Poland**: `legalcode-nda-triage-pl` — KC articles, UZNK trade secret framework, kary
  umowne analysis
...
- [ ] Add local legal terminology alongside English translations (if non-English
      jurisdiction)
- [ ] Verify DTSA or equivalent whistleblower protection requirements
- [ ] Check AI/data processing regulations affecting confidentiality provisions

---

## Provenance

Enhanced by Legalcode (2026-02-28). Original skill imported from
[anthropics/knowledge-work-plugins](https://github.com/anthropics/knowledge-work-plugins)
(`legal/commands/triage-nda.md`, Apache-2.0). Enhanced to Legalcode quality standard
incorporating:

- **Original triage-nda**: 13-criterion screening structure, GREEN/YELLOW/RED
  classification, market-standard defaults, multi-format input handling
- **legalcode-nda-triage**: 10-criteria deep analysis, interactive CLARIFY pattern,
  Glass Box audit trail, Citation Quality Gates, Self-Interrogation, Confidence Scoring,
  negotiation priority framework, playbook loading with defaults, anti-patterns catalogue,
  writing standards
- **legalcode-contract-review**: Quality assurance frameworks (PDCA, Citation Gates,
  Self-Interrogation, Confidence Scoring), stakeholder impact mapping, redline format
  standards

**New in this enhancement:**

- Criterion 11 (AI/Technology Provisions) — screening for AI/ML training restrictions,
  generative AI output handling, model decommissioning
- DTSA whistleblower immunity notice compliance (18 U.S.C. § 1833(b))
- Stakeholder impact mapping adapted for NDA context
- oneNDA standard-form benchmarking reference
- Marking requirements enforcement risk analysis
- Trade secret duration bifurcation analysis
- 17 anti-patterns (expanded from 14)
- AI-specific common issue analysis with redline guidance

Research sources:

- DTSA whistleblower immunity: 18 U.S.C. § 1833(b) [VERIFY]
- EU Trade Secrets Directive: Directive (EU) 2016/943 [VERIFY]
- oneNDA standard form: https://www.onenda.org/ (6,000+ organizations, ~10M agreements)
- Residuals clause enforceability: federal court precedent on trade secret licenses [VERIFY]
- AI NDA provisions: emerging market practice research (2024-2025)
